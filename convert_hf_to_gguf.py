...
@Model.register("MitForCausalLM")
class MitModel(Model):
    model_arch = gguf.MODEL_ARCH.MIT
...
@Model.register("MitVLForConditionalGeneration")
class MitVLModel(Model):
    model_arch = gguf.MODEL_ARCH.MITVL
...
@Model.register("MitMoeForCausalLM")
class MitMoeModel(Model):
    model_arch = gguf.MODEL_ARCH.MITMOE
...
@Model.register("RWKV6MitForCausalLM")
class RWKV6MitModel(Rwkv6Model):
    model_arch = gguf.MODEL_ARCH.RWKV6MIT
    ...
    def set_gguf_parameters(self):
        ...
        # special parameters for time_mixing in RWKV6MIT
        self.gguf_writer.add_layer_norm_rms_eps(rms_norm_eps)
        self.gguf_writer.add_token_shift_count(1)
        # RWKV6MIT use grouped key/value like GQA
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        for new_name, data in super().modify_tensors(data_torch, name, bid):
            if "time_mix_w1" in new_name or "time_mix_w2" in new_name:
                ...
                # rwkv6mit has a different order of rkvwg instead of the original wkvrg
